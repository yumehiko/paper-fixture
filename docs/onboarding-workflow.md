...
flowchart TD
    ...
    subgraph agent[agent と CLI]
        ...
        L["コマンド実行: 別 Blender プロセスで検証<br/>verify_assembly_bundle.py"]
    end

    subgraph bundle[生成された assembly bundle]
        M["assembly.blend"]
        N["textures/print-front.png"]
        O["build-manifest.json / verification.json<br/>preview 画像"]
    end
...
- fold がない部材は `build_assembly_bundle.py`、対応する直線 fold を持つ部材は `build_fold_assembly.py` を使います。いずれも生成後に別 Blender プロセスで bundle を再オープンして検証します。
- `assembly.blend` は `textures/print-front.png`、manifest、検証結果、preview と同じ bundle ディレクトリで扱います。Blender で編集・保存した bundle は、元の検証済み成果物とは異なる状態になるため、入力の修正では新しい出力先へ再生成します。
