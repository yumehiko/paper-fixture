...
def verify_instance(item, image_path):
    ...
    bounds = list(object_["pf_source_bounds_mm"])
    thickness = float(object_["pf_thickness_mm"])
    ...
    holes_open = [hole_is_open(object_, hole, bounds) for hole in json.loads(object_["pf_source_holes"])]
    ...
    print_range = list(object_["pf_print_range_mm"])
    if uv is None:
