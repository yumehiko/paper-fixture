...
class FoldAssemblyIntegrationTests(unittest.TestCase):
    def test_chain_keeps_curve_hole_and_nonzero_bounds(self):
        ...
        with tempfile.TemporaryDirectory(prefix="fold-chain-") as raw:
            ...
            self.assertLess(verified["roots"][0]["max_rotation_element_error"], verified["roots"][0]["rotation_element_tolerance"])
            self.assertLess(verified["roots"][0]["max_translation_error_m"], verified["roots"][0]["translation_tolerance_m"])
